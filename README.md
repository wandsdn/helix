...
Modules of the Helix MCSDN system. Helix is an OpenFlow built on the Ryu
framework. Helix defines two controller types, local and root controllers. A
local controller connects to and interacts with data plane devices. The root
controller connects to other local controllers and coordinates
inter-domain/inter-area operations.
...
This section describes the interfaces we call in YATES to simulate Helix's
path computation and TE algorithm. The interfaces import and override the
...
compute paths or perform TE),  simulating Helix’s behaviour. _The interfaces
...
The interfaces define several supported actions that require different state
provided as serialized JSON files (generated by YATES) or arguments (run the 
interface with the `--help` to see list of supported arguments). When executed,
the interfaces will load the provided state, perform the specified action,
...
All interfaces generate temporary files that contain state that needs to be
...
* `yates_interface.py` _DEPRECATED_
    * Old interfaces that allows testing Helix in single controller mode. This
      interfaces is partially deprecated. To test performance of Helix using a
      single controller, define a map that contains a single device connected
      to all switches in the topology and use `yates_mctrl_interface.py`.
* `yates_mctrl_interface.py` (Extends: `ProactiveController.py`, `TE.py`)
    * Helix local controller interface that allows simulating local controller
      ...
      JSON file (`yates_mctrl_interface.py --help` for list of arguments).
* `yates_root_interface.py` (Extends: `RootCtrl.py`)
    * Helix root controller interfaces.
...
./EmulateCtrlFail.py --topo <topo> --sw_ctrl_map <map> --scenario <scen> \
    ...
    --ctrl_log_level <ctrl_log_level> --disable_te
    ...
    [ctrl_log_level] - Custom logger level to use for the Helix controllers
        (defaults to "critical). Similar to framework output but applies to
        the controller temp files (generated during the experiment).
    --disable_te - Flag. Disable TE optimisation of controller (pass flag to
        run command. Only applies to Helix).
```

...
#   c2.1, c2.2 Failure Detection: [Last LE Fail Detect] - [Last Fail Action]
#   c2.1, c2.2 Role Change: [Last OF Role Change Req] - [Last LE Fail Detect]
#   c2.0 Instance Start: [Last LE Send Find] - [Last Start Action]
#   c2.0 Instance Init Phase: [Last LE Role Change] - [Last LE Send Find]
#   c2.0 SW Enter: [Last Last SW Enter] - [Last LE Send Find]
#   c2.0 Role Change: [Last OF Role Change Req] - [Last SW Enter]
...
if __name__ == "__main__":
            ...
            if found_first and scen == 0:
                ...
                if ev_type == "action":
                    ...
                    if ev_info == "inst_fail":
                        if cid not in data[run]:
                            data[run][cid] = {}
                        if "fail_detect" not in data[run][cid]:
                            data[run][cid]["fail_detect"] = {}
                        largest_time(data[run][cid]["fail_detect"],
                                        ev_ext, time)
                    elif ev_info == "role":
                        if cid not in data[run]:
                            data[run][cid] = {}
                        ...
                        # Send the initiation phase end time
                        if cid == "c2.0":
                            largest_time(data[run][cid], "init_end", time)

                    elif ev_info == "send_find":
                        if cid not in data[run]:
                            data[run][cid] = {}
    ...
    print("run,CID,Value,Detected Failure Of,Fail Action Time,LE Detect Time")
    dat_list = []
    for run, run_d in data.items():
        if ("c2.0" not in run_d or "c2.2" not in run_d or
                "c2.2" not in run_d or "fail_action" not in run_d["c2.0"] or
                "fail_action" not in run_d["c2.1"] or
                "fail_detect" not in run_d["c2.1"] or
                "fail_detect" not in run_d["c2.2"]):
            print("MISSING CID KEYS or FIELDS: RUN %s" % run)
            continue

        for cid in ["c2.1", "c2.2"]:
            cid_d = run_d[cid]
            for key in sorted(cid_d["fail_detect"].keys()):
                action_key = "c2.%s" % key
                fd_time = cid_d["fail_detect"][key]
                fa_time = run_d[action_key]["fail_action"]
                diff = fd_time - fa_time
                dat_list.append(diff)
                print("%s,%s,%s,%s,%s,%s" % (run, cid, diff, action_key,
                                                fa_time, fd_time))
    ...
    dat_list = []
    for run, run_d in data.items():
        if ("c2.1" not in run_d or "c2.2" not in run_d or
                "fail_detect" not in run_d["c2.1"] or
                "0" not in run_d["c2.1"]["fail_detect"] or
                "role_change" not in run_d["c2.2"] or
                "1" not in run_d["c2.2"]["fail_detect"]):
            print("MISSING CID KEYS or FIELDS: RUN %s" % run)
            continue

        # Compute the C2.1 failure detection
        fd_time = run_d["c2.1"]["fail_detect"]["0"]
        rc_time = run_d["c2.1"]["role_change"]
        diff = rc_time - fd_time
        dat_list.append(diff)
        print("%s,c2.1,%s,%s,%s" % (run, diff, fd_time, rc_time))

        # Compute the C2.2 failure detection
        fd_time = run_d["c2.2"]["fail_detect"]["1"]
        rc_time = run_d["c2.2"]["role_change"]
        diff = rc_time - fd_time
        dat_list.append(diff)
        print("%s,c2.2,%s,%s,%s" % (run, diff, fd_time, rc_time))
